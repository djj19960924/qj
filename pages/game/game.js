...
Page({
          ...
          if (this.data.human.status == 1 && this.data.humanB.status == 1) {
            ...
            if (this.data.index < this.data.titleNumber+1) {
              ...
              wx.onSocketMessage((res)=> {
                ...
                if (this.data.winner === this.data.openId) {
                  console.log('oooookokokokokok')
                  let award = this.data.humanScore * 20
                  wx.showModal({
                    title: '',
                    content: `挑战成功,获得${award}`,
                    cancelText: '取消',
                    cancelColor: '',
                    confirmText: '确定',
                    confirmColor: '',
                  ...
                  let award = this.data.humanScore * 10
                  wx.showModal({
                    title: '',
                    content: `挑战失败,获得${award}`,
                    success: function (res) {
                      wx.reLaunch({
                        url: '../sorts/sorts',
                      })
  ...
  humanDx(e) {
    ...
    if (!this.data.isMachine) {
      ...
      wx.sendSocketMessage({   //human每选一次答案，就会把自己的答案发送过去
        data: JSON.stringify({ from: this.data.openId, humanAnswer: humanAnswer, status: 10 })
      })
    }else{
      wx.sendSocketMessage({
  ...
  onLoad: function (options) {
    ...
    console.log('page:game:levelNum:', levelNum)
    ...
    console.log('llll:', this.data.userInfo.avatarUrl)
    ...
    console.log('titleNumber:', this.data.titleNumber)
  }
})